...
def generar_estilos_tabla(col_calendario, columnas_fin_semana):
    ...
    estilos_celdas_condicionales = [
        ...
        {
            'if': {'column_id': col},
            # SE HA AMPLIADO EL ANCHO DE 55px a 75px
...
def layout():
    ...
    if df_maestro.empty:
        return html.Div("No hay datos en el cronograma.", style={'color': '#DB563A', 'fontFamily': "'Outfit', sans-serif", 'padding': '20px'})
    ...
    return html.Div([
        ...
        html.H3("CRONOGRAMA DEL EQUIPO", style={'color': '#FF4E00', 'fontSize': '12px', 'textTransform': 'uppercase', 'fontFamily': "'Outfit', sans-serif", 'marginBottom': '24px'}),
        # --- BARRA DE FILTROS SUPERIOR ---
        html.Div([
            html.Div([
                html.Label("Código Licitación", style={'fontWeight': 'bold', 'color': '#474751', 'fontSize': '9px', 'textTransform': 'uppercase', 'marginBottom': '4px'}),
                dcc.Input(id="filtro-codigo", type="text", placeholder="Ej: LIC-2026...", 
                          style={'border': '1px solid #474751', 'padding': '8px 12px', 'fontFamily': "'Outfit', sans-serif", 'color': '#474751', 'outline': 'none'})
            ], style={'display': 'flex', 'flexDirection': 'column', 'marginRight': '24px', 'width': '200px'}),
            
            html.Div([
                html.Label("Nombre Proyecto", style={'fontWeight': 'bold', 'color': '#474751', 'fontSize': '9px', 'textTransform': 'uppercase', 'marginBottom': '4px'}),
                dcc.Input(id="filtro-nombre", type="text", placeholder="Buscar por nombre...", 
                          style={'border': '1px solid #474751', 'padding': '8px 12px', 'fontFamily': "'Outfit', sans-serif", 'color': '#474751', 'outline': 'none'})
            ], style={'display': 'flex', 'flexDirection': 'column', 'width': '250px'})
            
        ], style={
            'display': 'flex', 'alignItems': 'flex-end', 'marginBottom': '32px', 
            'padding': '24px', 'border': '1px solid #474751', 'backgroundColor': '#F0EEED'
        }),
        
        # --- TABLA: LICITACIONES NO ASIGNADAS ---
        html.H3("Licitaciones Pendientes de Asignación", style={'color': '#DB563A', 'fontSize': '12px', 'textTransform': 'uppercase', 'fontFamily': "'Outfit', sans-serif", 'marginBottom': '12px', 'fontWeight': 'bold'}),
        ...
        # --- TABLA: EN ESTUDIO ---
        html.H3("Etapa: En Estudio", style={'color': '#FF4E00', 'fontSize': '12px', 'textTransform': 'uppercase', 'fontFamily': "'Outfit', sans-serif", 'marginBottom': '12px'}),
        dash_table.DataTable(
            id='tabla-estudio', 
            columns=todas_cols, 
            fixed_columns={'headers': True, 'data': 8},
        ...
        html.H3("Etapa: Estudio Previo", style={'color': '#FF4E00', 'fontSize': '12px', 'textTransform': 'uppercase', 'fontFamily': "'Outfit', sans-serif", 'marginBottom': '12px'}),
        dash_table.DataTable(
            id='tabla-previo', 
            columns=todas_cols, 
            fixed_columns={'headers': True, 'data': 8},
